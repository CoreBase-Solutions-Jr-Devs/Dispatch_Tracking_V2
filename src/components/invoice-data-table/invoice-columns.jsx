...
function renderDateTime(value) {
	return <span className="text-muted-foreground font-mono text-sm">{formatDateTime(value)}</span>;
}
...
export function getInvoiceColumns(view) {
	var base = {
		...
		items: {
			...
			header: "Items",
			cell: function ({ row }) {
				return <span>{row.original.items ? row.original.items.length : 0}</span>;
			}
		},
		assignedDate: {
