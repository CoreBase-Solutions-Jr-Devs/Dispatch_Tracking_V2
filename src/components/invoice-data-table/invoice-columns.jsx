...
const renderDateTime = (value, position = 1) => {
  ...
  const baseColor = formattedDate === "—" ? "text-muted-foreground" : position === 1 ? "text-foreground" : "text-muted";
  return <span className={`${baseColor} font-mono text-sm`}>{formattedDate}</span>;
};
...
const renderActions = (row, handlers = {}, view) => {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <EditStatusDialog rowData={row.original} view={view} onSubmit={(updatedData) => console.log("Edited row data:", updatedData)}>
        <Button variant="outline" size="sm" className="h-8 w-8 p-0 hover:bg-accent" onClick={(e) => e.stopPropagation()}>
...
export function getInvoiceColumns(view) {
  const base = {
    ...
    postingDate: { accessorKey: "postingDate", header: "Posting Date & Time", cell: ({ row }) => renderDateTime(row.original.postingDate) },
    ...
    duration: { accessorKey: "duration", header: "Duration", cell: ({ row }) => renderDuration(row.original.duration, 62) },
    ...
    branchName: {
      ...
      header: "Branch Name",
      cell: ({ row }) =>
        row.original.docType === "TRANSFER" ? renderText(row.original.branchName) : <span className="text-muted-foreground italic">—</span>,
    },
