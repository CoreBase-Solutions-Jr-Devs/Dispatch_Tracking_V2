...
const renderStatus = (status) => (
	<Badge
		variant="outline"
		className={`${STATUS_STYLES[status] || "bg-muted text-muted-foreground border-border"} w-28 justify-center rounded-md text-xs px-3 py-1 font-medium border`}
	>
		{status}
	</Badge>
);
...
const renderDateTime = (value, position = 1) => {
	...
	if (formattedDate === "—") {
		return <span className="text-muted-foreground font-mono text-sm">{formattedDate}</span>;
	}
	...
	return (
		<span className={`${baseColor} font-mono text-sm`}>
			{formattedDate}
		</span>
	);
}
...
const renderDuration = (durationMinutes, avgMinutes = 30) => {
	...
	return (
		<span className={`font-medium ${isAbove ? "text-red-500" : "text-green-500"}`}>
