...
export default function FilterSheet() {
    ...
    const { startDate, endDate, dateRange } = useSelector(state => state.invoice);
    ...
    const formatDDMMYYYY = (date) => date ? new Date(date).toLocaleDateString("en-GB") : null;
    ...
    useEffect(() => {
        ...
        switch (dateRange) {
            case "TODAY":
                ...
                break;
            case "LAST_7_DAYS":
                start = new Date(today); start.setDate(today.getDate() - 6); end = new Date();
                break;
            case "LAST_30_DAYS":
                start = new Date(today); start.setDate(today.getDate() - 29); end = new Date();
                break;
            case "LAST_90_DAYS":
                start = new Date(today); start.setDate(today.getDate() - 89); end = new Date();
                break;
            default:
                return;
        }
    ...
    const handleDateChange = (type, date) => {
        ...
        type === "start" ? dispatch(setStartDate(formatted)) : dispatch(setEndDate(formatted));
    };
            ...
            <SheetContent side="right" className="w-full sm:max-w-md py-8 px-6">
                ...
                <SheetFooter className="border-t border-border flex justify-between items-center gap-x-4">
                    ...
                    <div className="flex gap-2">
                        <Button variant="apply" size="sm" onClick={handleApplyFilter}>Apply</Button>
